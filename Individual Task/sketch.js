...
class Element {
  ...
  update(sizeFactor, radiusFactor) {
    if (
      this.isInCenterArea(this.x, this.y, width / 2, height / 2, radiusFactor)
    ) {
  ...
  isInCenterArea(x, y, centerX, centerY, radiusFactor) {
    return (
      Math.sqrt(Math.pow(x - centerX, 2) + Math.pow(y - centerY, 2)) <=
      (width / 2) * radiusFactor
    );
  }
}
...
function setup() {
  ...
  button.mousePressed(play_stop);
}

function draw() {
  clear();
  let level = amp.getLevel();
  let sizeFactor; // Element size scaling factor
  let radiusFactor; // Amplitude effect coverage factor
  let colorFactor; // Background color transformation factor
  if (song.isPlaying()) {
    // Transform background color based on amplitude
    colorFactor = map(level, 0, 0.5, -1, 1);
    if (colorFactor > 0) {
      h += 0.3;
      h = constrain(h, 0, 360);
    ...
    sizeFactor = map(level, 0, 0.5, 0.5, 2);
  ...
  for (let pp of rParents) {
    pp.display();
  }
  for (let cc of rChildren) {
    cc.update(sizeFactor, radiusFactor);
    cc.display();
  }
}
